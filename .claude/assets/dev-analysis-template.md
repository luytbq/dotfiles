...
| STT | Nội dung | Ngày | Trang | Chi tiết | Người phê duyệt |
|-----|----------|------|-------|----------|-----------------|
| 1 | Tạo mới | {DATE} | | | |
...
(
  ...
  - xem "Note về cách viết Graylog filter" ở phần dưới để biết thêm chi tiết về cách viết filter
...
( 
  Note về cách viết Graylog filter: 
  - filter log_service: là tên service gọi log, ví dụ: wsp, msp, prsp, tsp, psp-onecredit, psp-connector-homecredit
  - filter log_class: là tên class gọi log, ví dụ: vn.onepay.msp.resources.applepay.ApplePayments
  - có thể dùng phép OR để check vài điều kiện một lúc hoặc tách ra thành nhiều filter riêng biệt nếu cần thiết
)
